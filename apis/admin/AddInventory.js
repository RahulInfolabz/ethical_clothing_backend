...
async function AddInventory(req, res) {
  try {
    ...
    // Check if inventory entry already exists for this item + size combination
    const exists = await db.collection("inventory").findOne({
      item_id: new ObjectId(item_id),
      size_id: new ObjectId(size_id),
    });

    if (exists) {
      return res.status(400).json({ success: false, message: "Inventory entry already exists for this item and size" });
    }
